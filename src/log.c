...
int parse_loglevel(const char *name) {
    ...
    PRINT_LOG_WARN("Could not parse loglevel, setting default");
    return LOG_INFO;
}
...
const char *lookup_loglevel(int level) {
    ...
    PRINT_LOG_WARN("Could not lookup loglevel");
    return "";
}
...
void set_loglevel(int level) {
    ...
    PRINT_LOG_NOTICE("Setting loglevel to %s", loglevel_names[level]);
    loglevel = level;
}
